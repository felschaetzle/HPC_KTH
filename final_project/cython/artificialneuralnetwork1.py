...
def predict(Theta1, Theta2, X):
	...
	a2 = cythonfn.g(a1 @ Theta1.T)                 # apply sigmoid: input layer --> hidden layer
	a2 = np.hstack((np.ones((m,1)), a2))  # add bias (hidden layer)
	a3 = cythonfn.g(a2 @ Theta2.T)                 # apply sigmoid: hidden layer --> output layer
...
def cost_function(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda, a1):
	...
	a2 = cythonfn.g(a1 @ Theta1.T)                 
	a2 = np.hstack((np.ones((m,1)), a2))  
	a3 = cythonfn.g(a2 @ Theta2.T)                 
...
def gradient(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda, a1):
	...
	delta3 = a3 - y_k
	delta2 = (delta3 @ Theta2[:, 1:]) * cythonfn.grad_g(z2)
